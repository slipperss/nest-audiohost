...
export class UsersService {
	...
	async uploadAvatar(file: Express.Multer.File, user: User): Promise<User> {
		const avatar = await this.fileService.uploadFile(file.buffer)

		// deleting prev user avatar from storage
		await this.fileService.deleteFile(user.avatar)

		return await this.update({ id: user.id }, { avatar: avatar })
	}
	...
	async getUserById(id: number, req_user: User) {
		const user = await this.getOne({
			where: { id: id },
			relations: { followers: true, albums: true, tracks: true },
		})
