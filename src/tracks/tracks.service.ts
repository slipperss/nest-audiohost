...
export class TracksService {
	...
	async create(
		...
		if (music_file) {
			const musicFile = await this.fileService.uploadFile(music_file.buffer)
			...
			if (image_file) {
				const imageFile = await this.fileService.uploadFile(image_file.buffer)
			...
			return await this.getOne({
				where: { id: track.id },
				relations: { author: true, audio: true, image: true },
			})
		}
	...
	async update(
		criteria: FindOptionsWhere<Track>,
		partialEntity: QueryDeepPartialEntity<Track>,
	): Promise<Track> {
		try {
	...
	async uploadTrackAvatar(
		...
		const trackAvatar = await this.fileService.uploadFile(image_file.buffer)

		// Delete previous track avatar image
		await this.fileService.deleteFile(track.image)

		track.image = trackAvatar
	...
	async delete(id: number): Promise<any> {
		...
		const musicFilePath = path.resolve(__dirname, '..', 'static/' + track.audio)
		fs.rm(musicFilePath, () => {})
	...
	async addTrackListening(id: number, user: User) {
		...
		// Check if user already listened this track
		if (
			!user.listenedTracks.some(
				(listenedTrack) => listenedTrack.id === track.id,
			)
		) {
